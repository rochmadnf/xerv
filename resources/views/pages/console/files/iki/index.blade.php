...
@section('content')
    <div class="flex items-end justify-between gap-4">
        <h1 class="text-2xl/8 font-semibold text-zinc-950">Berkas IKI</h1>
        <a href="{{ route('files.iki.create') }}"
    ...
    <div class="mt-8 space-y-4">
        ...
        @forelse ($files as $file)
            <div class="relative flex w-full flex-row items-center gap-x-3 rounded-lg border border-slate-300 p-4">
                <div class="size-6 rounded-full bg-blue-300 text-center font-medium text-white">
                    {{ $no++ }}</div>
                <div class="flex flex-1 shrink-0 flex-col justify-between gap-y-1 border-x-2 px-2">
                    <div class="space-y-0.5">
                        ...
                        <a href="{{ asset('assets/' . $file->document_path) }}" target="_blank"
                            ...
                            <h3>Indeks Kinerja Individu &mdash;
                                {{ trim("{$file?->user?->user_detail?->front_title} {$file?->user?->name} {$file?->user?->user_detail?->back_title}") }}
                            </h3>
                        </a>
                    </div>
                </div>
                <div id="deleteModal-{{ $file->id }}"
                    class="absolute right-0 z-[9999] hidden items-center justify-center bg-white">
                    <div class="space-y-4 rounded-lg border border-slate-500/50 bg-white p-10">
                        <p class="border-b border-b-slate-300 pb-2 font-medium uppercase">Menghapus Data?</p>
                        <form action="{{ route('files.iki.destroy', ['id' => $file->uuid]) }}" method="POST"
                ...
                <div class="relative flex w-36 flex-row items-center justify-center gap-x-2">
                    {{-- Edit Data --}}
                    <div>
                        <a href="{{ route('files.iki.edit', ['id' => $file->uuid]) }}" type="button"
